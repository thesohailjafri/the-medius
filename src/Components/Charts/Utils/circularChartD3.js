...
export const circularChartD3 = (dataObject, mainSvg, scaled=1) => {
    let dataObj = {...dataObject, data: [dataObject.data]}
    let svg = mainSvg.append('g')
        .attr('class', 'parent')
        .append('g')
        .attr('transform',
        'translate(' + (80* scaled) + ',' + (80 * scaled) + ') scale(' + scaled +')')
    ...
    let initFunc = (data, title) => {
        ...
        svg.append("text")
            ...
            .text(dataObject.data.value + (!dataObject.data.value ? '' : '%'))
