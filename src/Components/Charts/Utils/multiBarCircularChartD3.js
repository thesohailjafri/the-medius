...
export const multiBarCircularChartD3 = (dataObj, mainSvg, scaled=1) => {

    let svg = mainSvg.append('g')
        .attr('class', 'parent')
        .append('g')
        .attr('transform',
        'translate(' + (260* scaled) + ',' + (110 * scaled) + ') scale(' + scaled +')')
    ...
    let arcMinRadius = arcMapRadius[dataObj.data.length]
    let arcPadding = arcMapPadding[dataObj.data.length]
    ...
    let initFunc = (data, title) => {
        ...
        let arcs = d3.arc()
            .innerRadius((d, i) => getInnerRadiusPoint(i) + 6)
            .outerRadius((d, i) => getOuterRadiusPoint(i) - 6)
        ...
        let legendContainer = mainSvg.selectAll('.parent')
            ...
            .attr('transform',
                'translate(' + (30 ) + ','
                + (30  + legendPadding[dataObj.data.length]) + ')')
        ...
        legend
            .attr('transform', (d, i) => {
                var x = -paddingLegend;
                var y = Math.floor(i) * paddingLegend * 2.5 -10
        ...
        legend
            ...
            .attr('y', legendItemSize - legendSpacing + 8)
            .style("font-size", "14px")
        ...
        legend
            .append('text')
            .attr('x', legendItemSize + legendSpacing)
            .attr('y', legendItemSize - legendSpacing + 26)
            .style("font-size", "12px")
            .attr('fill', '#727272')
            .text((d) => d.name)
